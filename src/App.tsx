    ...
    <div className="h-screen w-screen overflow-hidden text-slate-100 font-sans flex flex-col bg-slate-950">
      ...
      <header className="h-8 flex items-center justify-between px-3 bg-white/5 border-b border-white/10 shrink-0 z-50">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`flex items-center gap-1 px-2 py-0.5 rounded text-[8px] font-bold border ${
              audioStatus === 'running'
                ...
                : 'bg-amber-600/20 text-amber-400 border-amber-500/30 animate-pulse'
            }`}
          >
          ...
          <button onClick={() => setShowInfo(true)} className="p-1 text-slate-400">
            <Info size={12} />
          </button>
      ...
      {/* Main — fila horizontal que ocupa todo el espacio restante */}
      ...
      <footer className="h-5 shrink-0 bg-white/5 border-t border-white/10 flex items-center justify-between px-3 z-40">
        ...
        <span className="text-[7px] text-slate-600 font-mono">v1.4.0</span>
      </footer>
        ...
        {showInfo && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              className="bg-slate-900/90 backdrop-blur-2xl p-6 rounded-2xl border border-white/10 max-w-sm w-full relative"
            >
              <button onClick={() => setShowInfo(false)} className="absolute top-4 right-4 text-slate-400">
                <X size={18} />
              </button>
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                <Headphones className="text-indigo-400" size={18} />
                Cómo usar VirtualDeck
              </h3>
              <ul className="space-y-3 text-slate-400 text-xs">
                ...
                <li className="flex gap-2"><span className="text-indigo-400 font-mono font-bold">02.</span><span>Usa el <b>Crossfader</b> del centro para mezclar entre plato A y B.</span></li>
                <li className="flex gap-2"><span className="text-indigo-400 font-mono font-bold">03.</span><span>Ajusta el <b>Pitch</b> para cambiar la velocidad. Usa <b>Sync</b> para igualar tempos.</span></li>
                <li className="flex gap-2"><span className="text-indigo-400 font-mono font-bold">04.</span><span>Pulsa <b>Rec</b> para grabar tu sesión.</span></li>
              </ul>
              <button onClick={() => setShowInfo(false)} className="w-full mt-5 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-400 font-bold py-2 rounded-xl border border-indigo-500/30 text-sm transition-all">
                Cerrar
              </button>
            </motion.div>
          </motion.div>
        ...
        {showAcademy && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
              className="bg-slate-900 p-6 rounded-2xl border border-pink-500/20 max-w-sm w-full relative"
            >
              ...
              <button onClick={() => setShowAcademy(false)} className="w-full bg-pink-600 hover:bg-pink-500 text-white font-black uppercase tracking-widest py-3 rounded-xl text-sm transition-all">
                Aceptar reto
              </button>
            </motion.div>
          </motion.div>
